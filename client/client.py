...
class Client(object):
    ...
    def send_request(self, service, args={}, file_args=None):
        ...
        try:
            ...
            with open('C:\\Users\\tejas\\OneDrive\\Documents\\GitHub\\Star_Tracker\\subid.txt') as myfile:
                if 'None' in myfile.read():
                    file = open("C:\\Users\\tejas\\OneDrive\\Documents\\GitHub\\Star_Tracker\\subid.txt", "w+")
                    subid = str(result.get('subid'))
                    file.write(subid)
                    file.close()
                    print('test')
                else:
                    myfile.close()
            print(result.get('subid'))
    ...
    def overlay_plot(self, service, outfn, wcsfn, wcsext=0):
        from astrometry.uticll import util as anutil
